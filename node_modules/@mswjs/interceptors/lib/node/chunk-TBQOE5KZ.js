...
var _chunkQFLITAVVjs = require('./chunk-QFLITAVV.js');
...
var MockHttpSocket = class extends MockSocket {
  constructor(options) {
    ...
    this.onRequestStart = (versionMajor, versionMinor, rawHeaders, _, path, __, ___, ____, shouldKeepAlive) => {
      ...
      const headers = _chunkQFLITAVVjs.FetchResponse.parseRawHeaders(rawHeaders);
      ...
      const requestId = _chunkQFLITAVVjs.createRequestId.call(void 0, );
      ...
      Reflect.set(this.request, _chunkQFLITAVVjs.kRawRequest, Reflect.get(this, "_httpMessage"));
      if (this.request.headers.has(_chunkQFLITAVVjs.INTERNAL_REQUEST_ID_HEADER_NAME)) {
        this.passthrough();
        return;
      }
      this.onRequest({
    ...
    this.onResponseStart = (versionMajor, versionMinor, rawHeaders, method, url, status, statusText) => {
      const headers = _chunkQFLITAVVjs.FetchResponse.parseRawHeaders(rawHeaders);
      const response = new (0, _chunkQFLITAVVjs.FetchResponse)(
        /**
         ...
         */
        _chunkQFLITAVVjs.FetchResponse.isResponseWithBody(status) ? _stream.Readable.toWeb(
      ...
      _chunkQFLITAVVjs.FetchResponse.setUrl(this.request.url, response);
      if (this.request.headers.has(_chunkQFLITAVVjs.INTERNAL_REQUEST_ID_HEADER_NAME)) {
        return;
      }
      ...
      if (writeArgs !== void 0) {
        if (!headersWritten) {
          ...
          const requestHeadersString = rawRequestHeaders.filter(([name]) => {
            return name.toLowerCase() !== _chunkQFLITAVVjs.INTERNAL_REQUEST_ID_HEADER_NAME;
...
// src/interceptors/ClientRequest/index.ts
var _ClientRequestInterceptor = class extends _chunkQFLITAVVjs.Interceptor {
  constructor() {
...
//# sourceMappingURL=chunk-TBQOE5KZ.js.map